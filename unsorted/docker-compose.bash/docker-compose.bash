...
b2sync)
    ...
    EXCLUDE_DIRS=("unity-accelerator/agent")
    B2_EXCLUDE_PARAMS=()
    for exdir in "${EXCLUDE_DIRS[@]}"; do
        B2_EXCLUDE_PARAMS+=("--exclude" "$exdir/**")
    done
    ...
    rclone sync "$ROOT_DIR/$dir" "$RCLONE_REMOTE/$dir" --progress "${B2_EXCLUDE_PARAMS[@]}" --checkers=32 --transfers=32
